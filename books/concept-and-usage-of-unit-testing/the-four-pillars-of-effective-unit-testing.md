...
では具体的に 2 つをどのように扱えばよいでしょうか？
→ 単体テストでは、**迅速なフィードバックを最優先しつつ、適切なリグレッションに対する保護を確保する**という方針が適切です。

次のようなプラクティスを実践すると良いでしょう。

1. **迅速なフィードバックを実現するために**
   - プロセス外依存（DB、外部 API、ファイルシステムなど）をテストダブルで置き換える
   - テストの実行時間を数ミリ秒〜数秒程度に抑える
   - 単一の振る舞いに焦点を当て、テストの範囲を限定する
2. **最低限のリグレッションに対する保護を備えるために**
   - ビジネスロジックを含む重要な部分をテストする
   - 境界値や異常系のケースもカバーする
   - ただし、プロセス外依存を含む包括的な検証は統合テストに委ねる

:::message
**「リグレッションに対する保護」において、
単体で備えられない部分は統合以上のテストサイズで補う。**

![](https://storage.googleapis.com/zenn-user-upload/e1260836ae8c-20250426.png)
:::
