...
単体テストの構成要素である「**隔離された状態で実行される**」の意味が何となく分かってきたのではないでしょうか？
...
| 派閥         | 隔離対象     | 単体の意味                                                       | テスト・ダブルの置き換え対象 |
| ------------ | ------------ | ---------------------------------------------------------------- | ---------------------------- |
| ロンドン学派 | 単体         | 1 つのクラス                                                     | 不変依存を除くすべての依存   |
| 古典学派     | テストケース | 1 つのクラス、もしくは、同じ目的を達成するためのクラスのグループ | 共有依存                     |

:::message
**🧪 ロンドン学派と古典学派における結合テストの違い**
:::

## 「振る舞い」を単位にテストせよ

https://zenn.dev/openlogi/articles/unit-test-principles#%E3%81%AF%E3%81%98%E3%82%81%E3%81%AB
...
なぜでしょうか？
↓ ↓ ↓
**プロジェクトの持続的な成長を促すことに向いているからです。**

以下はロンドン学派のメリットです。
しかしこれらは、単体テストの目的である「プロジェクトの持続的な成長を促す」という観点で考えると、懸念が存在します。
...
2. **テスト失敗の原因特定が簡単**
   ...
   **🤔 懸念点**
   ...
   - テストを頻繁に実行していれば、古典学派であっても原因の特定は難しくない
   - 頻繁に実行していれば、テストに失敗した場合、変更したコードが原因であるため、確認すべきスコープは明確
