...
- **攻撃用 JavaScript が、攻撃対象サイトとは別のサイト（罠サイト）や攻撃者が用意した URL にある場合の攻撃手法**
...
具体例として、ニュースサイトに存在する検索機能を利用した攻撃の流れを解説する。

1. **攻撃者が攻撃リンクを用意**
   - 攻撃対象のニュースウェブサイトには、ユーザーがキーワードを入力してニュース記事を検索できる機能があり、以下のような URL になっている
     - `http://example-news.com/search?q=keyword`
   - 攻撃者は、この検索機能に XSS 脆弱性が存在することを見抜き、以下のように悪意あるスクリプトを含むリンクを作成する
     - `http://example-news.com/search?q=<script>alert('XSS');</script>`
2. **攻撃者が正規ユーザーを誘導**
   - 攻撃者は、作成した悪意あるリンクを、電子メールやソーシャルメディアを通じて、ニュースサイトのユーザーに送る
   - メッセージには、「興味深いニュース記事をチェックしてください！」のような誘導文が含まれているかもしれない
3. **正規ユーザーがリンクをクリック**
   - 受け取ったリンクをクリックしたユーザーは、攻撃者が用意した悪意あるスクリプトを含む検索リクエストをニュースサイトに送信する
4. **攻撃対象のサイトでスクリプトが実行**
   - もしニュースウェブサイトがユーザーからの入力を適切にエスケープしていなければ、検索クエリに含まれていた悪意あるスクリプトがそのままウェブページに表示され、ブラウザで実行される
   - この例では、`alert('XSS');` のスクリプトが実行され、ユーザーのブラウザに「XSS」という警告メッセージが表示される
...
- 持続型 XSS は、Web メールや SNS などが典型的な攻撃ターゲット
...
- フロントエンド側のコード不備が原因となって脆弱性が発生する
- フロント側の JavaScript はデベロッパーツールでコードの中身を見ることができるため、攻撃者に狙われやすい

### DOM について

- DOM は HTML を操作するためのインターフェース
- ブラウザは HTML 構文を解析して DOM ツリーと呼ばれる構造体を生成する
- 生成された DOM ツリーは JavaScript で変更することが可能
- DOM ツリーの内容が変化すれば、元となる HTML も書き換わるため、JavaScript で画面の表示を変更できる

### DOM-based XSS が発生する例
...
以下は URL から `こんにちは` の文字列を取得し、id が `message` の要素へ挿入するコード。
...
挿入された文字列は `<img>` 要素としてブラウザに解釈され、`onerror` 属性に指定された `location.href='https://attacker.example` のスクリプトが実行される。

この例では、**`innerHTML` を使用した DOM 操作**が DOM-based XSS の原因。

攻撃の流れは以下の図の通り。

![](https://storage.googleapis.com/zenn-user-upload/76bd06553c88-20240306.png)

### DOM-based XSS 脆弱性の原因

- ブラウザが持つ機能の使用が原因
- 原因になるブラウザの機能は「**ソース**」と「**シンク**」に分類できる
- ソースとして動作する機能の代表例は以下の通り
  - `location.hash`
  - `location.search`
  - `document.referrer`
  - `postMessage`
...
- シンクとして動作する機能の代表例は以下の通り
  ...
  - `jQuery()`
- これらの機能を利用する際には注意が必要
- 与えるデータに対して適切に処理を行えば XSS は発生しない
