...
1. **初期化コマンド: `npx create-rext [プロジェクト名]`**
   ...
   初期化後、すぐに CLI を使った操作やモジュールの利用を始められます。
...
3. **モジュール機能**
   このパッケージでは、`reverseText` 関数をモジュールとして提供します。
...
npm パッケージの名前は、npm レジストリ上でユニークである必要があります。
パッけープリター名はスペースを入れず、小文字にする必要があります。（`-`, `.`, `_` は OK）
...
スコープとは、npm パッケージ名にプレフィックスを追加することで、パッケージをグループ化する方法です。
...
`npm init` を使って、パッケージのメタデータを管理するための `package.json` を作成します。

:::message
そもそも「パッケージ」とは、`package.json` を含むファイルやディレクトリのことです。
そのため、npm レジストリに公開するパッケージには、`package.json` が含まれている必要があります。
:::

以下は、`npm init` を実行すると、以下のプロンプトが表示されます。
...
\
パッケージの説明を記述できます。
このパッケージが何を提供するものなのか？どのように使用するのかを記述します。
...
3. **ビルドの設定**
   JS にコンパイルする必要があるので、`package.json` を修正します、
...
ユーザーが `npx create-rext [プロジェクト名]` コマンドを実行した際、テンプレートの内容が指定したフォルダー（`[プロジェクト名]/`）にコピーされます。
...
npm パッケージで CLI コマンドを提供する場合、`package.json` の `bin` フィールドを設定する必要があります。
...
- **キー（`create-rext`）**
  ユーザーが実行する CLI コマンド名を指定します。
  この場合、`npx create-rext` または `create-rext` が実行可能になります。
