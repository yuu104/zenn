...
- 段階を分けてイメージのビルドができる機能
- 各段階では異なるベースイメージを指定できる
- 各段階の任意の成果物を以降のビルド段階にコピーすることもできる
-

## .dockerignore

`.dockerignore` は、Docker ビルドプロセス中に無視するファイルやディレクトリを指定するためのファイル。
Git の `.gitignore` ファイルと似た役割を果たすが、Docker イメージのビルドに特化している。

### 目的

1. **ビルドコンテキストの最適化**
   `.dockerignore` に指定されたファイルやディレクトリは、Docker デーモンに送信されるビルドコンテキストから除外される。これにより、ビルドコンテキストのサイズを小さくし、ビルド時間を短縮できる。
2. **機密情報の保護**
   機密情報が含まれるファイル（`.env` など）をビルドコンテキストから除外することで、これらの情報が誤ってイメージに含まれないようにする。

:::details ビルドコンテキストとは？

イメージビルドの際に Docker デーモンに送られるファイルとディレクトリのセットのこと。
ビルドコンテキストは Docker イメージを作成するために必要なすべてのファイル（ソースコード、Dockerfile、依存ファイルなど）を含むディレクトリ。

:::

:::details Docker デーモンとは？

Docker のバックグラウンドプロセスであり、Docker コンテナの作成、管理、実行を担当する主要なコンポーネント。
`dockerd` とも呼ばれ、通常はシステムの起動時に開始され、バックグラウンドで実行される。

:::

### 記述方法

ビルドコンテキストから除外したいファイルやディレクトリを指定する。
各行に 1 つのパターンを記述する。
...
1. **ビルドコンテキストの指定**
   `build` コマンドを実行する際に、ビルドコンテキストのパス（`.` など）を指定する。
   `dockerignore` ファイルに記述されたパターンに一致するファイルやディレクトリは、ビルドコンテキストから除外される。
2. **ビルドの実行**
   Docker はビルドコンテキストを基にしてイメージをビルドする。
   `.dockerignore` ファイルにより除外されたファイルやディレクトリは、ビルドプロセス中に無視される。

### ベストプラクティス

https://qiita.com/techneconn/items/6428b7a3bab1893499f0
