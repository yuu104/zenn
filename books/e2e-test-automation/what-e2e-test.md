...
アプリを実装したあと、ログインをしてみたり、画面のボタンをクリックしてみたり、フォームに入力して内容が反映されているか確認したりといった、動作確認を行っていると思います。
その動作確認として行った操作をそのまま E2E テストとして、実装するのです。
**「テストが存在する」という状態を作ることが一番重要です。**
...
E2E テストを含むテストコード全般において、効果的な設計方針を採用することは非常に重要です。
プロダクトコードとは異なる考え方が必要な場面もあり、テスト特有の方針も存在します。
ここでは、テストコードを書く際に意識すべき主要な設計方針について解説します。
...
3. **必要最小限の構造化**
   ...
   - 同じ準備ロジックや片付けの処理がある場合は、`test.describe()`, `test.beforeEach()`, `test.afterEach()`などを使用
