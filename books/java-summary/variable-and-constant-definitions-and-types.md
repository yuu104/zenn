...
プリミティブ型はメモリサイズがあらかじめ決まっている。

| プリミティブ型 | メモリサイズ（ビット） | メモリサイズ（バイト） | 説明                            |
| -------------- | ---------------------- | ---------------------- | ------------------------------- |
| boolean        | JVM 依存               | JVM 依存               | 真偽値（`true` または `false`） |
| byte           | 8                      | 1                      | 符号付き 8 ビット整数           |
| short          | 16                     | 2                      | 符号付き 16 ビット整数          |
| char           | 16                     | 2                      | Unicode 文字                    |
| int            | 32                     | 4                      | 符号付き 32 ビット整数          |
| long           | 64                     | 8                      | 符号付き 64 ビット整数          |
| float          | 32                     | 4                      | 単精度 32 ビット浮動小数点数    |
| double         | 64                     | 8                      | 倍精度 64 ビット浮動小数点数    |
