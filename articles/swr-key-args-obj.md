...
`key` は第二引数にあるフェッチ関数に渡すパラメータであり、取得データのキャッシュを識別するためのキーとしても使用されます。
「取得データのキャッシュを識別する」ため、`key` にはユニークな識別子を渡す必要があります。
`key` には API エンドポイントなどの**文字列**を指定することが一般的です。
...
`key` に文字列を指定した場合は、その値をキーとしてキャッシュデータを識別します。
しかし、上記のようにブジェクト型の値を `key` パラメータに指定した場合、キャッシュキーはどうなるのでしょうか？
...
どうやら、オブジェクト型の値を`key`パラメータに指定した場合、その値を基に SWR が内部でシリアライズし、文字列に変換してキャッシュキーを生成しているようです。
...
今回着目する `key` パラメータは `_key` になりますね。

そして、下記の部分で `_key` を `serialize() ` 関数の引数に指定し、シリアライズしているようです。
関数の戻り値は、上部のコメントによると
...
- `key` が関数の場合 : 関数を実行し、実行結果をキャッシュキーとする
...
こちらの関数では、引数として受け取った`arg`（`key` パラメータ）を以下に分類して処理しています。
...
これにより、同じ値のオブジェクトや配列が `useSWR()` の `key` に指定された場合でも、再度ハッシュ化の処理をしなくても済むようにしています。
