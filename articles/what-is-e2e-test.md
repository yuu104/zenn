...
2. **開発者がテストによるフィードバックを素早く受けられるようにする**
   自動テストは（実行できる環境を用意すれば）**これまで作ってきたものが、今でも正常に動作している**ことを、いつでも素早く教えてくれます。
   自分で手動テストしたり、テスターにテストしてもらうのを待つ必要はありません。
...
つまり、手動テストと自動テストはそれぞれ異なる強みを持っており、相互に補完し合う関係にあると言えます。
